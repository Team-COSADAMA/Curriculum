...
4. **오경보율(FPR, FalsePositiveRate)**: 찾고자 하는 것이 아닌 0에 대해 1이라고 **얼마나 잘못 예측 했는지를 나타내는 지표**. 0에 가까울수록 좋은 모델. 민감도가 커지면 FP가 커질 수 있으며 오경보율 또한 커지게 됩니다. **이 관계를 곡선으로 나타낸 것이 아래 서술할 ROC curve**입니다.
